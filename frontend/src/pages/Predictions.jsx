...
import { Lock, Trophy, Zap, ShieldAlert, Check, AlertCircle, ChevronDown } from "lucide-react";
...
const Predictions = () => {
  ...
  const [raceRound, setRaceRound]   = useState(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const round = next.round ?? (races.indexOf(next) + 1);
        setNextRace(next);
        setRaceRound(round);
        ...
        addToast("Failed to load race data.", "error");
      } finally {
  ...
  async function handleSave(andLock = false) {
    ...
    try {
      ...
      if (!upsertRes.ok) {
        ...
        addToast(err.error ?? "Failed to save prediction.", "error");
        return;
      }
      ...
      if (andLock) {
        ...
        if (!lockRes.ok) {
          ...
          addToast(err.error ?? "Failed to lock prediction.", "error");
          return;
        }
  ...
  const gpName          = nextRace ? (nextRace.raceName ?? nextRace.circuit?.circuitName ?? "Next Race") : "";
  ...
  if (!nextRace) {
    return (
      <div className="container mx-auto max-w-3xl px-4 py-8 text-center">
        ...
        <h1 className="mb-2 font-display text-3xl">SEASON CONCLUDED</h1>
        <p className="text-muted-foreground">Predictions will open when the new season begins.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-8">
      ...
      >
        ...
        <p className="mb-8 text-muted-foreground">{gpName}</p>
          ...
          <div className="glass-card mb-6 border border-podium-green/30 p-5">
            ...
            <div className="grid grid-cols-3 gap-3 text-sm">
              {[
                ...
                <div
                  key={label}
                  className={`rounded-xl p-3 ${predicted === actual ? "border border-podium-green/30 bg-podium-green/10" : "bg-muted"}`}
                >
                  <div className="mb-1 text-xs text-muted-foreground">{label}</div>
                  <div className="font-semibold">{driverName(predicted)}</div>
                  {predicted !== actual && (
                    <div className="mt-1 text-xs text-muted-foreground">Actual: {driverName(actual)}</div>
                  )}
                  {predicted === actual && (
                    <div className="mt-1 text-xs text-podium-green">+{pts} pts</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            {SC_OPTIONS.map(({ label, value }) => (
              <button
                key={value}
                type="button"
                onClick={() => !locked && setSafetyCar(value)}
                disabled={locked}
