...
const SkeletonGrid = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    {Array.from({ length: 10 }).map((_, i) => (
      <div key={i} className="glass-card h-28 animate-pulse" style={{ animationDelay: `${i * 60}ms` }} />
    ))}
  </div>
);
...
const RaceCard = ({ race, index, status, onClick }) => {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={[
        "glass-card p-5 text-left w-full cursor-pointer transition-all duration-200 hover-lift",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-start gap-4 min-w-0">
          <span className={`font-display text-5xl leading-none tabular-nums shrink-0 ${isCompleted ? "text-muted-foreground/40" : "text-primary"}`}>
            {String(race.round).padStart(2, "0")}
          </span>
          <div className="min-w-0">
            <div className="text-xl mb-0.5">{flag}</div>
            <div className="font-display text-sm leading-snug">
              {(race.raceName ?? race.circuit?.circuitName ?? "").replace(" Grand Prix", " GP")}
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
              <MapPin className="h-3 w-3 shrink-0" />
              <span className="truncate">{race.circuit?.city}, {race.circuit?.country}</span>
            </div>
    ...
    <div className="flex items-center gap-4">
      <div className={`flex h-9 w-9 items-center justify-center rounded-xl shrink-0 ${
        ...
        {isPast
          ...
          : <Clock className={`h-4 w-4 ${isNext ? "text-primary" : "text-muted-foreground"}`} />
        }
      </div>
      <div>
        <div className={`font-semibold text-sm ${isNext ? "text-primary" : ""}`}>
          {session.label}
        </div>
        {session.formatted && (
      ...
      {!loading && !weather && (
        <div className="text-center py-6">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Forecast available<br />within 16 days of race day.
          </p>
        </div>
      )}
        ...
        return (
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { Icon: Droplets,    label: "Rain",     value: `${weather.rain ?? "—"}%` },
                { Icon: Wind,        label: "Wind",     value: `${weather.wind != null ? Math.round(weather.wind) : "—"} km/h` },
                { Icon: Thermometer, label: "Humidity", value: `${weather.humidity != null ? Math.round(weather.humidity) : "—"}%` },
              ].map(({ Icon, label: lbl, value }) => (
                <div key={lbl} className="rounded-xl bg-muted/50 p-3 text-center">
...
const DetailView = ({ race, onBack, weather, weatherLoading }) => {
  ...
  const tzLabel = timezone === "UTC" ? "UTC" : timezone.split("/")[1]?.replace(/_/g, " ") ?? timezone;
  ...
  const sessionList = Object.entries(race.schedule ?? {})
    ...
    .map(([key, s]) => {
      ...
      return { key, label: SESSION_META[key].label, dt, formatted: dt ? formatLocalTime(dt, timezone) : null };
    })
    .sort((a, b) => {
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h1 className="font-display text-3xl sm:text-5xl mb-2 leading-none">
                {(race.raceName ?? race.circuit?.circuitName ?? "").toUpperCase()}
              </h1>
                ...
                <div className="flex flex-wrap gap-2 mt-5">
                  {c.circuitLength && (
                    <div className="px-3 py-1.5 rounded-xl bg-muted/50 text-xs">
                      ...
                      <span className="font-mono font-semibold">{c.circuitLength} km</span>
                    </div>
                  )}
                  {c.numberOfCorners && (
                    <div className="px-3 py-1.5 rounded-xl bg-muted/50 text-xs">
                      <span className="text-muted-foreground">Corners </span>
                      <span className="font-mono font-semibold">{c.numberOfCorners}</span>
                    </div>
                  )}
                  {c.lapRecord && (
                    <div className="px-3 py-1.5 rounded-xl bg-muted/50 text-xs">
                      <span className="text-muted-foreground">Lap Record </span>
                      <span className="font-mono font-semibold">{c.lapRecord}</span>
                    </div>
                  )}
                  <div className="px-3 py-1.5 rounded-xl bg-muted/50 text-xs">
            ...
            <div className="lg:col-span-3">
              ...
              <div className="space-y-3">
                {sessionList.length === 0 && (
                  <p className="text-sm text-muted-foreground">No schedule data available yet.</p>
                )}
...
const GridView = ({ races, onSelect }) => {
  ...
  const completed = races.filter(r => { const d = getRaceDateTime(r); return d && d < now; }).length;
    ...
    <div className="container mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="mb-8 text-muted-foreground text-sm">
            ...
            <span className="text-secondary font-semibold">{completed}</span>/{races.length} races completed
          </p>
...
const Schedule = () => {
  ...
  useEffect(() => {
    fetch("https://f1api.dev/api/2026")
      ...
      .catch(() => { setError("Couldn't load the 2026 calendar. Please try again."); setLoading(false); });
  }, []);
